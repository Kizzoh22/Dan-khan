...
const { imageToWebp, videoToWebp, writeExifImg, writeExifVid } = require('./lib/dan_khanexif');
const { smsg, isUrl, generateMessageTag, getBuffer, getSizeMedia, fetchJson, await, sleep } = require('./lib/dan_khanfunc');
...
async function startdan_khan() {
  ...
  const client = dan_khanConnect({
    logger: pino({ level: "silent" }),
    printQRInTerminal: true,
    browser: ["DAN_KHAN-XMD", "Safari", "5.1.7"],
    auth: state,
  ...
  client.ev.on("messages.upsert", async (chatUpdate) => {
    try {
      ...
      if (autolike === 'TRUE' && mek.key && mek.key.remoteJid === "status@broadcast") {
        const dan_khanii = await client.decodeJid(client.user.id);
        await client.sendMessage(mek.key.remoteJid, { react: { key: mek.key, text: '🎭' } }, { statusJidList: [mek.key.participant, Dan_khanii] });
      }

      if (!client.public && !mek.key.fromMe && chatUpdate.type === "notify") return;

      let m = smsg(client, mek, store);
      const dan_khan = require("./dan_khan");
      dan_khan(client, m, chatUpdate, store);
    } catch (err) {
      console.log(err);
    }
  });
  ...
  client.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      console.log(color("Congrats, DAN_KHAN-XMD has successfully connected to this server", "green"));
      console.log(color("Follow me on Instagram as Nick_hunter9", "red"));
      console.log(color("Text the bot number with menu to check my command list"));
      client.sendMessage(client.user.id, { text: `> 𝗕𝗼𝘁 𝐢𝐬 𝗼𝗻𝗹𝗶𝗻𝗲【DAN_KHAN-𝗫𝗠𝗗】𝗶𝗳 𝘆𝗼𝘂 𝗻𝗲𝗲𝗱 𝗮𝗻𝘆 𝗵𝗲𝗹𝗽, 𝘁𝗲𝐱𝐭 𝗺𝗲 𝘁𝗵𝗿𝗼𝘂𝗴𝗵 +254798956113 𝗼𝗿 𝗮𝗻𝘆 𝗶𝘀𝘀𝘂𝗲` });
    }
  });
 ...
 const getBuffer = async (url, options) => {
  ...
  client.sendFile = async (jid, PATH, fileName, quoted = {}, options = {}) => {
    ...
    if (options.asSticker || /webp/.test(mime)) {
      let { writeExif } = require('./libdan_khanexif.js');
